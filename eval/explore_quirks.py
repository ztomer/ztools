...
TEST_PROMPTS = {
    ...
    "structured": {
        "system": (
            "Output ONLY valid JSON array. Each item must have: name, "
            "location, target_ages, price, weather, day. No text before or "
            "after."
        ),
        "user": (
            "Spring Festival: Downsview Park, all ages, free. Coding: "
            "Library, 8-14, $20. Farmers Market: Maple Village, all ages, "
            "free."
        ),
    },
    "ultra_strict": {
        "system": (
            'CRITICAL: Output ONLY a JSON array. Nothing else. No intro '
            'text. No markdown. Format: '
            '[{"name":"value","location":"value"}]'
        ),
        "user": (
            "Extract to JSON: Spring Festival at Downsview Park, Indoor "
            "Coding at Library, Farmers Market at Maple Village"
        ),
    },
}
...
def run_test(model: str, test_name: str, timeout: int = DEFAULT_TEST_TIMEOUT) -> dict:
    ...
    try:
        ...
        analysis = {
            ...
            "has_details": sum(1 for p in (parsed or []) if has_item_details(p))
...
- Farmers Market: Fresh produce. April 20. All ages."""
    ...
    try:
        ...
        # Check details
        with_details = (
            sum(1 for p in parsed if has_item_details(p)) if isinstance(parsed, list) else 0
        )
...
def explore_model(model: str, timeout: int = DEFAULT_TEST_TIMEOUT):
    ...
    for test_name in TEST_PROMPTS:
        ...
        if result.get("error"):
            ...
            print("  WARN: Has markdown in output")
            status = "PARTIAL"
        else:
            print("  FAIL: No JSON parsed")
            status = "FAIL"
    ...
    print(
        f"  Items: {src_result.get('items', 0)}, with details: {src_result.get('with_details', 0)}"
    )
    ...
    print(
        f"  [SOURCE] {src_result.get('status')} "
        f"(match: {src_result.get('source_match_ratio', 0):.0%})"
    )
