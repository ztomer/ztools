...
class TestWeekendExclusions:
    ...
    def test_exclude_by_location(self):
        ...
        items = [{"name": "Shop", "location": "Westfield mall"}, {"name": "Park", "location": "Green"}]
        s = fn(json.dumps(items), case)
        assert s.score == 50.0
...
class TestWeekendScorersIntegration:
    def test_tasks_register_correctly(self):
        from lib.quality_scorers import get_scorers, TASK_SCORERS
    ...
    def test_full_scoring_workflow(self, mock_llm):
        ...
        ref = json.dumps({"age_range": [6, 13], "weather": {"sat": "rain"}, "exclude": [], "source_item_names": ["Park"]})
        case = make_case(input_text="Park data", reference=ref)
        items = json.dumps([{"name": "Park", "location": "Main", "target_ages": "6-12", "price": "free", "weather": "indoor", "day": "sat"}])
