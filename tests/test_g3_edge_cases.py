...
# --------------------------------------------------------------------------
# C18 root cause: constrain candidates BEFORE the structure phase
# --------------------------------------------------------------------------


def test_out_of_window_candidates_are_set_aside_before_the_draft():
    """Traced: 7 events reached the structure phase, all dated outside the
    weekend, and the final window filter killed every one. The model must be
    choosing among events that CAN happen."""
    from weekend.enforce import filter_candidates_to_window

    draft = "\n".join([
        "Kite Festival | Pier 4 | August 8-9 | $12 | 5-12 | kites",
        "Night Market | Concord | August 2 | free | all | stalls",
        "Jurassic Quest | Mississauga | August 7-9 | $25 | 3-12 | dinos",
        "Museum Late | Toronto | July 18 | $10 | 8+ | tour",
    ])
    kept, dropped = filter_candidates_to_window(draft, date(2026, 8, 7), date(2026, 8, 9))
    assert dropped == 2
    assert "Kite Festival" in kept and "Jurassic Quest" in kept
    assert "Night Market" not in kept and "Museum Late" not in kept


def test_an_undated_candidate_is_kept_because_dropping_is_not_free():
    from weekend.enforce import filter_candidates_to_window

    draft = "Summer Splash | Vaughan | unknown | unknown | unknown | water play"
    kept, dropped = filter_candidates_to_window(draft, date(2026, 8, 7), date(2026, 8, 9))
    assert dropped == 0 and "Summer Splash" in kept


def test_an_exhibition_spanning_the_weekend_is_kept():
    """Overlap, not containment -- the same rule the final filter uses."""
    from weekend.enforce import filter_candidates_to_window

    draft = "Long Exhibit | Toronto | June 29 to August 16 | $20 | all | art"
    kept, dropped = filter_candidates_to_window(draft, date(2026, 8, 7), date(2026, 8, 9))
    assert dropped == 0 and "Long Exhibit" in kept


def test_a_line_that_is_not_in_carrier_format_is_left_alone():
    from weekend.enforce import filter_candidates_to_window

    draft = "some prose the model emitted instead of a table row"
    kept, dropped = filter_candidates_to_window(draft, date(2026, 8, 7), date(2026, 8, 9))
    assert dropped == 0 and kept == draft


def test_an_impossible_date_is_blanked_not_rendered():
    """Probed on the real model: a DATES field of "unknown" produced 2026-08-00,
    a syntactically plausible and semantically impossible date."""
    from weekend.enforce import blank_unparseable_dates

    items = [{"name": "Splash", "start_date": "2026-08-00", "end_date": "2026-13-40"}]
    out, notes = blank_unparseable_dates(items, 2026)
    assert out[0]["start_date"] == "" and out[0]["end_date"] == ""
    assert len(notes) == 2


def test_a_readable_date_survives_blanking():
    from weekend.enforce import blank_unparseable_dates

    items = [{"name": "Kite", "start_date": "2026-08-08", "end_date": ""}]
    out, notes = blank_unparseable_dates(items, 2026)
    assert out[0]["start_date"] == "2026-08-08" and notes == []
