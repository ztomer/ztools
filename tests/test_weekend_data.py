...
class TestGetWeekendDateObjects:
    def test_returns_friday_and_sunday(self):
        from weekend_data import get_weekend_date_objects
...
class TestGetWeekendDatesString:
    def test_format(self):
        from weekend_data import get_weekend_dates_string
...
class TestFetchWeather:
    def test_successful_forecast(self):
        from weekend_data import fetch_weather
        friday = datetime.date(2026, 4, 10)
        sunday = datetime.date(2026, 4, 12)
        ...
        with patch("weekend_data.requests.get", return_value=mock_response) as mock_get:
    ...
    def test_fallback_on_exception(self, capsys):
        ...
        friday = datetime.date(2026, 4, 10)
        sunday = datetime.date(2026, 4, 12)
        with patch("weekend_data.requests.get", side_effect=Exception("network")):
            result = fetch_weather(friday, sunday)
    ...
    def test_empty_daily(self):
        ...
        with patch("weekend_data.requests.get", return_value=mock_response):
            result = fetch_weather(friday, sunday)
        assert "Daily Forecast" in result
...
class TestFetchTransientEvents:
    def test_successful(self):
        from weekend_data import fetch_transient_events
        mock_results = [
        ...
        with patch("weekend_data.DDGS", fake_ddgs):
...
class TestFetchFixedVenues:
    def test_successful(self):
        from weekend_data import fetch_fixed_venues
        mock_results = [
...
class TestScrapeReviewScore:
    def test_match_slash_format(self):
        from weekend_data import scrape_review_score
        mock_results = [
