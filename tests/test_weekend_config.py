...
class TestCacheFunctions:
    def test_load_events_cache_exists(self, tmp_path, monkeypatch):
        from weekend_config import load_events_cache
        cache_file = tmp_path / ".weekend_events_debug_cache.json"
        cache_file.write_text('["event1", "event2"]')
        monkeypatch.setattr("weekend_config.DEBUG_EVENTS_FILE", cache_file)
    ...
    def test_save_events_cache(self, tmp_path, monkeypatch):
        from weekend_config import save_events_cache
        cache_file = tmp_path / ".weekend_events_debug_cache.json"
        monkeypatch.setattr("weekend_config.DEBUG_EVENTS_FILE", cache_file)
    ...
    def test_load_venues_cache_exists(self, tmp_path, monkeypatch):
        from weekend_config import load_venues_cache
        cache_file = tmp_path / ".weekend_venues_debug_cache.json"
        cache_file.write_text('["venue1"]')
        monkeypatch.setattr("weekend_config.DEBUG_VENUES_FILE", cache_file)
    ...
    def test_save_venues_cache(self, tmp_path, monkeypatch):
        from weekend_config import save_venues_cache
        cache_file = tmp_path / ".weekend_venues_debug_cache.json"
        monkeypatch.setattr("weekend_config.DEBUG_VENUES_FILE", cache_file)
...
class TestLoadWeekendConfig:
    def test_load_existing(self):
        from weekend_config import load_weekend_config
...
class TestServerHelpers:
    def test_is_server_running_ours(self):
        from weekend_config import is_server_running_ours
        with patch("weekend_config.is_server_running", return_value=True):
            assert is_server_running_ours() is True

    def test_is_server_running_ours_false(self):
        from weekend_config import is_server_running_ours
        with patch("weekend_config.is_server_running", return_value=False):
            assert is_server_running_ours() is False

    def test_restart_osaurus(self):
        from weekend_config import restart_osaurus
        with patch("weekend_config.restart_server", return_value=True) as mock_restart:
    ...
    def test_ensure_server(self):
        from weekend_config import ensure_server
        with patch("weekend_config._osaurus_ensure_server", return_value=True) as mock_ensure:
