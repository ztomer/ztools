...
class TestPrintHelpers:
    def test_print_header(self, capsys):
        ...
        fake_console = Console(
            file=StringIO(), force_terminal=True, width=200, force_interactive=True
        )
        with patch("weekend.output.console", fake_console):
...
class TestBuildFixedTable:
    def test_empty(self):
        from weekend.output import _build_fixed_table
...
class TestBuildMarkdownTables:
    def test_full_build(self):
        ...
        with patch("weekend.output.fetch_scores_for_items", side_effect=add_score):
    ...
    def test_structured_data_list(self):
        ...
        with patch("weekend.output.fetch_scores_for_items", return_value=None):
            result = build_markdown_tables(
                dates_str="April",
                weather_str="",
