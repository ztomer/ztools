...
class TestWeekendMainFlow:
        ...
        try:
            with (
                ...
                patch.object(
                    wp,
                    "_fetch_data",
                    return_value=(FAKE_WEATHER, FAKE_EVENTS, FAKE_VENUES, FAKE_DATES),
                ),
                patch.object(
                    wp, "generate_weekend_plan", return_value=(FAKE_TRANSIENT, FAKE_FIXED)
                ) as mock_gen,
            ):
    ...
    def test_main_low_items_triggers_warning(self, mock_llm, capsys, tmp_path):
        ...
        with (
            ...
            patch.object(
                wp, "generate_weekend_plan", return_value=(few_items, {"fixed_activities": []})
            ),
        ):
            wp.main(_make_args())
        captured = capsys.readouterr()
...
class TestWeekendFetchData:
    ...
    def test_fetch_data_basic(self, mock_llm):
        ...
        with (
            ...
            weather, events, venues, dates = wp._fetch_data(
                fri, sun, "2026", "June", use_cache=False
            )
    ...
    def test_fetch_data_uses_cache_hit(self, mock_llm):
        ...
        with (
            ...
            weather, events, venues, dates = wp._fetch_data(
                fri, sun, "2026", "June", use_cache=True
            )
        assert events == FAKE_EVENTS
        assert venues == FAKE_VENUES
...
class TestWeekendParseFunctions:
    ...
    def test_parse_fixed_list(self):
        ...
        result = wp._parse_fixed([{"name": "Place 1"}, {"name": "Place 2"}], "mock-model", {})
        assert len(result) == 2
    ...
    def test_parse_fixed_dict_with_key(self):
        ...
        result = wp._parse_fixed({"fixed_activities": [{"name": "Place 1"}]}, "mock-model", {})
        assert len(result) == 1
        assert result[0]["name"] == "Place 1"
    ...
    def test_parse_fixed_single_object(self):
        ...
        result = wp._parse_fixed({"name": "Single Place", "location": "Toronto"}, "mock-model", {})
        assert len(result) == 1
    ...
    def test_parse_fixed_fallback_key(self):
        ...
        result = wp._parse_fixed({"odd_key": [{"name": "Fallback Item"}]}, "mock-model", {})
        assert len(result) == 1
    ...
    def test_parse_transient_list(self):
        ...
        result = wp._parse_transient([{"name": "Event 1"}, {"name": "Event 2"}], "mock-model", {})
        assert len(result) == 2

    def test_parse_transient_dict_with_key(self):
        import weekend.cli as wp

        result = wp._parse_transient({"transient_events": [{"name": "Event 1"}]}, "mock-model", {})
        assert len(result) == 1
...
class TestWeekendPrompts:
    ...
    def test_build_fixed_system_prompt_with_args(self):
        ...
        prompt = build_fixed_system_prompt(
            model="mock-model", location="Vaughan/Toronto", age_range="4-12"
        )
        assert "Output JSON now" in prompt
        assert "fixed_activities" in prompt
